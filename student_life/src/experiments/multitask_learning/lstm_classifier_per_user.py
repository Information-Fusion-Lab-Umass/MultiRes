...
for split_no, split in enumerate(splits):
    ...
    for student_id in student_list:
        ...
        optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate,
                                     weight_decay=decay)
        model_dict[student_id] = (model, optimizer)
    ...
    for epoch in tqdm.tqdm(range(n_epochs)):
        ...
        (train_total_loss, train_total_reconstruction_loss, train_total_classification_loss,
         train_labels, train_preds, train_users) = trainer.evaluate_multitask_learner_per_user(tensorified_data,
                                                                                      'train_ids',
                                                                                      num_classes,
                                                                                      model,
        ...
        (val_total_loss, val_total_reconstruction_loss, val_total_classification_loss,
         val_labels, val_preds, val_users) = trainer.evaluate_multitask_learner_per_user(tensorified_data,
                                                                                'val_ids',
                                                                                num_classes,
                                                                                model,
                                                                                reconstruction_criterion,
