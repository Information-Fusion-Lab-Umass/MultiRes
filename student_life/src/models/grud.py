"""
Python module that defines GRUD NN module.
"""
import torch
import math
import warnings
...
class GRUD(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        @param input: Input of a tuple - (X, M, Delta)
        1. X - Data of shape (num_features x sequence length)
        The sequence length is mean to be constant, or should be equal to the minimum seq. length.
        2. M - Missing mask corresponding to data X. 0 when data is missing and 1 when present.
        3. Delta - Time delta between the current step and last observed value.
        @return: Y_pred of shape 1 X output_size (usually equal to num_classes)
        """
