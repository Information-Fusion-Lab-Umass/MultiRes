...
class Model(nn.Module):
    def __init__(self, params):
        ...
        self.lstm_slow = nn.LSTM(self.num_slow_feats, int(self.lstm_hidden_dim / 2), num_layers=self.layers,
                                 bidirectional=True, batch_first=True, dropout=self.dropout)
        self.lstm_moderate = nn.LSTM(self.num_moderate_feats, int(self.lstm_hidden_dim / 2), num_layers=self.layers,
                                     bidirectional=True, batch_first=True, dropout=self.dropout)
        self.lstm_fast = nn.LSTM(self.num_fast_feats, int(self.lstm_hidden_dim / 2), num_layers=self.layers,
                                 bidirectional=True, batch_first=True, dropout=self.dropout)
    ...
    def _forward(self, features, flag):
        ...
        self.hidden = self.init_hidden(batch_size,
                                       frequency_flag=flag)  # Tuple, length 2  Size of [0] and [1]: (2,1,d*10)
        ...
        if flag == 'fast':
            ...
            pad_attn = 0  # error # TODO: Check this
