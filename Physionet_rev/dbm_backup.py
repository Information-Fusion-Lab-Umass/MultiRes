...
#         self.imputation_layer_op = nn.Linear(self.imputation_layer_dim, 1)
        ...
        if(self.bilstm_flag):
            self.lstm_slow = nn.LSTM(self.input_dim, self.hidden_dim/2, num_layers = self.layers,
                                bidirectional=True, batch_first=True, dropout=self.dropout)
            self.lstm_fast = nn.LSTM(self.input_dim, self.hidden_dim / 2, num_layers=self.layers,
                                     bidirectional=True, batch_first=True, dropout=self.dropout)
        else:
            self.lstm_slow = nn.LSTM(self.input_dim, self.hidden_dim, num_layers = self.layers,
                                bidirectional=False, batch_first=True, dropout=self.dropout)
            self.lstm_fast = nn.LSTM(self.input_dim, self.hidden_dim, num_layers=self.layers,
                                     bidirectional=False, batch_first=True, dropout=self.dropout)

        self.hidden2tag = nn.Linear(2*self.hidden_dim, self.tagset_size)    # Multiply by 2 for the slow and fast vectors
        
        if(self.attn_category == 'dot'):
            print "Dot Attention is being used!"
            self.attn_slow = DotAttentionLayer(self.hidden_dim).cuda()
            self.attn_fast = DotAttentionLayer(self.hidden_dim).cuda()
    ...
    def forward_slow(self, features):
        ...
        # LSTM Layers
        self.hidden = self.init_hidden(batch_size)  # Tuple, length 2  Size of [0] and [1]: (2,1,d*10)
    ...
    # TODO: Fix hidden2tag (print weights just before and after for this); Add layers specific for fast and slow (in forward_slow and init)
    # TODO: Specific features include everything from DotAttention?



    def forward(self, data, id_):
#         features = self.LL(features)
        features = self.get_imputed_feats(data[id_][0], data[id_][1], self.dict_selected_feats)    # Size: (1, timesteps, d*10)

        # TODO: Divide features into fast and slow (call them features_fast, features_slow) For now,
        features_fast = features
        features_slow = features
        ...
        #print "HELLO", pad_attn_slow.shape, self.hidden_dim, self.tagset_size
        if(self.attn_category=='dot'):
            pad_attn = torch.cat((pad_attn_slow, pad_attn_fast), 1)
            #print "ATTN", pad_attn.shape
