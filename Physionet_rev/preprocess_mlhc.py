...
def prepare_data_block(slow_block_flag, all_tsteps):
    ...
    for i in range(len(all_tsteps)):
        if all_tsteps[i] in tsteps:
            ...
            if slow_block_flag:
                avg_moderate = get_average_values(time_range, 'moderate')  # (d_moderate,)
                if np.isnan(avg_moderate).any():
                    avg_moderate = np.average(moderate_data, axis=0)
            avg_fast = get_average_values(time_range, 'fast')  # (d_fast,)
        
            # If there is no fast signal present, impute with average of signal
            if np.isnan(avg_fast).any():
                avg_fast = np.average(fast_data, axis=0)
...
with open('processed_data.pkl', 'wb') as f:
    pickle.dump([final_data_train, final_data_val, final_data_test] , f)
