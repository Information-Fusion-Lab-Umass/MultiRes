...
class Model(nn.Module):
    def __init__(self, params):
        ...
        if params['lstm_output_type']  == 'different':      
        	self.hidden_dim_slow = 200 #params['hidden_constant']*self.num_slow_feats
        	self.hidden_dim_moderate = 200  #params['hidden_constant']*self.num_moderate_feats
        	self.hidden_dim_fast = 200  #params['hidden_constant']*self.num_fast_feats
	
        	self.lstm_slow = nn.LSTM(self.num_slow_feats, int(round(self.hidden_dim_slow / 2)), num_layers=self.layers, bidirectional=True, batch_first=True, dropout=self.dropout)
        	self.lstm_moderate = nn.LSTM(self.num_moderate_feats, int(round(self.hidden_dim_moderate / 2)), num_layers=self.layers, bidirectional=True, batch_first=True, dropout=self.dropout)
        	self.lstm_fast = nn.LSTM(self.num_fast_feats, int(round(self.hidden_dim_fast / 2)), num_layers=self.layers, bidirectional=True, batch_first=True, dropout=self.dropout)
