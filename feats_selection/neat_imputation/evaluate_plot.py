...
def evaluate_dbm(model, imputation_dict):
    preds = []
    actual = []
    for each_ID_t in imputation_dict.keys():
        label_scores = model(imputation_dict[each_ID_t]['data'])
        
        val, ind = torch.max(label_scores, dim=1)
        preds+=ind.tolist()
        
        labels = [imputation_dict[each_ID_t]['label']]
