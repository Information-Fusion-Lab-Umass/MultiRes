...
def incremental_select(corr_path, file_path):
    ...
    pickle.dump({'train_ids': data['train_ids'],
                 ...
                 'data': select_l.copy()}, open('../../data/features/osaka_1_fl.pkl', 'wb'))
    ...
    for i in range(1, len(idx_corr)):
        ...
        pickle.dump({'train_ids': data['train_ids'],
                     'test_ids': data['test_ids'],
                     'val_ids': data['val_ids'],
                     'data': select_l.copy()}, open('../../data/features/osaka_'+str(i+1)+'_fl.pkl', 'wb'))


if __name__ == '__main__':
    incremental_select('../../data/Osaka Features Corr - osaka_features_corr.csv', '../../data/osaka_py27.pkl')
