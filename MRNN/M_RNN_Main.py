...
if mode == 'Complete':
    ...
    _, trainZ, trainM, trainT, testX, testZ, testM, testT = Data_Loader_Complete(train_rate,
                                                                                 missing_rate)

    # 2. M_RNN_Imputation (Recovery)
    _, Recover_testX = M_RNN(trainZ, trainM, trainT, testZ, testM, testT)
    print(Recover_testX.shape)
...
if mode == 'Incomplete':
    ...
    print(trainZ.shape, trainM.shape, trainT.shape, testX.shape, testZ.shape, testM.shape, testT.shape)

    # 2. M_RNN_Imputation (Recovery on testX)
    Recover_trainX, Recover_testX = M_RNN(trainZ, trainM, trainT, testZ, testM, testT)

    # print(Recover_trainX, Recover_testX)
