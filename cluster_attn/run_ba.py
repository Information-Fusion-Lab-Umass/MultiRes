...
def preprocess(raw):
    ...
    for id_key in ids_dict.keys():
        ...
        for i in ids_dict[id_key]:
            ...
            feats = d[0]
            label = d[3]
            all_values.append(feats)
            all_labels.append(label)
        processed[id_key] = {'data': all_values, 'label': all_labels}

    return processed


def tri_fit(params, type, lr=0.0001):
    if type == 'mean':
        raw = [pickle.load(open('./data/phy_data_set_%d.pkl' % i, 'rb')) for i in range(1, 4)]

        pre = [preprocess(raw[i]) for i in range(0, 3)]
        trains = [pre[i]['train'] for i in range(0, 3)]
        tests = [pre[i]['test'] for i in range(0, 3)]
        vals = [pre[i]['val'] for i in range(0, 3)]
    elif type == 'forward':
        imputed = [pickle.load(open('./data/forward_physionet_%d.pkl' % i, 'rb')) for i in range(1, 4)]
